...
export default function MenuEditor({ store, t, F, onSave }: Props) {
  ...
  // food_categories + store_category_map 로드 후 flat 메뉴 구성
  useEffect(() => {
    Promise.all([
      ...
      // store_category_map: [{ store_category, food_category_id }]
      // store.categories: { sectionName: [menuNames] }
      ...
      const flat: FlatMenu[] = []
      Object.entries(store.categories || {}).forEach(([section, names]) => {
        const foodCategoryId = catToFoodId[section] || ''
        ;(names as string[]).forEach(nameKo => {
          flat.push({
            id: uid(),
            nameKo,
            ...
            foodCategoryId,
            options: (store.menu_options?.[nameKo] || []).map(opt => ({
              id: uid(), key: opt.key,
              labelKo: opt.labelKo, labelEn: opt.labelEn,
              choices: opt.choices.map((c: { ko: string; en: string; extraPrice?: string }) => ({
                id: uid(), ko: c.ko, en: c.en, extraPrice: c.extraPrice ?? '',
              })),
            })),
          })
        })
      })
      setMenus(flat.length > 0 ? flat : [EMPTY_MENU()])
    })
  }, [store])

  const updateMenu = (idx: number, updated: FlatMenu) => {
    const m = [...menus]; m[idx] = updated; setMenus(m)
  }
  const deleteMenu = (idx: number) => {
    if (menus.length > 1) setMenus(menus.filter((_, i) => i !== idx))
  }

  const handleSave = async () => {
    const validMenus = menus.filter(m => m.nameKo.trim())

    // food_category별 그룹핑 → stores.categories
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      {!preview && (
        <>
          ...
          {menus.map((menu, mi) => {
            ...
            return (
              <div key={menu.id} style={{ background: '#0d0d0d', border: '1px solid #1e1e1e', borderRadius: 12, marginBottom: 10, padding: '12px 12px 10px' }}>
                {/* 메뉴 입력 행 */}
                <div style={{ display: 'flex', gap: 7, alignItems: 'center', marginBottom: 8 }}>
                  <input value={menu.nameKo} onChange={e => update({ ...menu, nameKo: e.target.value })}
                    placeholder='메뉴명' style={{ flex: 2.5, ...inputSt, fontWeight: 700, ...F }} />
                  <input value={menu.nameEn} onChange={e => update({ ...menu, nameEn: e.target.value })}
                    placeholder='Menu (EN)' style={{ flex: 2.5, ...inputSt, color: '#c8a96e', ...F }} />
                  <div style={{ display: 'flex', alignItems: 'center', gap: 3, flex: 1.2 }}>
                    <input value={menu.price} onChange={e => update({ ...menu, price: e.target.value })}
                      placeholder='가격' style={{ flex: 1, ...inputSt, textAlign: 'right', ...F }} />
                    <span style={{ fontSize: 10, color: '#555', flexShrink: 0 }}>원</span>
                  </div>
                  <button onClick={() => deleteMenu(mi)} disabled={menus.length === 1}
                    style={{ width: 26, height: 26, borderRadius: '50%', border: 'none', background: menus.length === 1 ? '#141414' : '#2a1a1a', color: menus.length === 1 ? '#333' : '#e05a5a', cursor: menus.length === 1 ? 'default' : 'pointer', fontSize: 12, flexShrink: 0 }}>✕</button>
                </div>
                      ...
                      return (
                        <div key={opt.id} style={{ background: '#111', borderRadius: 10, padding: '10px 12px', marginBottom: 6, border: '1px solid #1e1e1e' }}>
                          <div style={{ display: 'flex', gap: 6, marginBottom: 8, alignItems: 'center' }}>
                            ...
                            <button onClick={() => update({ ...menu, options: menu.options.filter((_, i) => i !== oi) })}
                ...
                <button onClick={() => update({ ...menu, options: [...menu.options, { id: uid(), key: uid(), labelKo: '', labelEn: '', choices: [{ id: uid(), ko: '', en: '', extraPrice: '' }, { id: uid(), ko: '', en: '', extraPrice: '' }] }] })}
                  style={{ width: '100%', background: 'none', border: '1px dashed #1e1e1e', borderRadius: 7, padding: '6px', color: '#383838', fontSize: 11, cursor: 'pointer', marginTop: 8, ...F }}>
                  {t.addOption}
                </button>
              </div>
            )
          })}
          <button onClick={() => setMenus(m => [...m, EMPTY_MENU()])}
      ...
      {/* 미리보기 */}
      {preview && (() => {
        const grouped: Record<string, FlatMenu[]> = {}
        menus.filter(m => m.nameKo).forEach(m => {
          const fc = foodCategories.find(f => f.id === m.foodCategoryId)
          const key = fc ? `${fc.group_emoji} ${fc.name_ko}` : '기타'

...
export default function OwnerLoginPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) window.location.href = '/owner/dashboard'
    })
  ...
  const handleEmailAuth = async () => {
    ...
    if (mode === 'signup') {
      const { error } = await supabase.auth.signUp({
        email, password,
        options: { data: { name }, emailRedirectTo: `${window.location.origin}/owner/dashboard` }
      })
    ...
    <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, ...F }}>
      <div style={{ width: '100%', maxWidth: 380, background: '#0d0d0d', border: '1px solid #1e1e1e', borderRadius: 20, padding: 32 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <div style={{ fontSize: 11, color: '#555', letterSpacing: 3, marginTop: 4 }}>점주 포털</div>
        </div>
        ...
        {/* 구글 로그인 */}
        <button onClick={handleGoogle} style={{ width: '100%', padding: '11px', background: '#fff', color: '#333', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 16, ...F }}>
        ...
        {mode === 'signup' && (
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: '#666', marginBottom: 6 }}>이름 (가게명)</div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input type="email" value={email} onChange={e => setEmail(e.target.value)}
            ...
            placeholder="owner@example.com" style={inp} />
        </div>
